    ...
    private String fitnesseSh() {
        ...
        for (Vhost vhost : ports.vhosts()) {
            if (vhost.isWebapp()) {
                ...
                result.append("java -jar -Dtest.history.days=0 $HOME/fitnesse-standalone.jar -v -r src/test/fitnesse -p " + vhost.httpPort() + " -e 0 >/var/log/stool/fitnesse-" + vhost.name + ".log 2>&1 &\n");
