...
Any user may start a stage. The processes started by this command belong to the stage maintainer, which is not necessarily the user 
...
addition, Stool defines a `USER` variable set to the stage maintainer and a `HOME` variable pointing to its home directory.
