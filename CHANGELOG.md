...
* setup (many of them to make Stool 6 usable next to Stool 5)
  ...
  * simplified configuration: it's all in one `~/.stool.yaml` file now; the location of this file is configurable with
    the `STOOL_YAML` environment variable, `STOOL_HOME` is no longer supported. Shell completion and stage indicator code 
