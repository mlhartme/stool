...
public abstract class Stage {
    ...
    public static Stage load(Session session, FileNode backstageLink) throws IOException {
        ...
        return load(session, session.loadStageConfiguration(backstageResolved), backstageLink, backstageResolved.getParent());
    }

    private static Stage load(Session session, StageConfiguration configuration, FileNode backstageLink, FileNode directory) throws IOException {
        Stage result;
        String url;
        ...
        result = createOpt(session, url, configuration, backstageLink, directory);
        if (result == null) {
    ...
    public static Stage createOpt(Session session, String url, StageConfiguration configuration, FileNode backstageLink,
        ...
        if (url.startsWith("gav:") || url.startsWith("file:")) {
            return new ArtifactStage(session, url, backstageLink, directory, configuration);
        }
        if (directory.join(configuration.pom).exists()) {
            return SourceStage.forLocal(session, backstageLink, directory, configuration);
        }
        return null;
    }
     ...
     */
    public FileNode backstageLink;
    public FileNode backstageResolved;
    ...
    public Stage(Session session, String url, FileNode backstageLink, FileNode directory, StageConfiguration configuration) throws ReadLinkException {
        this.session = session;
        this.url = url;
        this.backstageLink = backstageLink;
        this.backstageResolved = backstageLink.resolveLink();
        this.directory = directory;
        this.configuration = configuration;
    }

    public FileNode shared() {
        return shared(backstageResolved);
    }

    /** User visible name of the stage */
    public String getName() {
        return backstageLink.getName();
    }

    public FileNode getBackstage() {
        return backstageResolved;
    }
    ...
    private Launcher serviceWrapper(String ... action) throws IOException {
        ...
        launcher.arg(backstageResolved.getAbsolute());
        launcher.arg(action);
        return launcher;
    }
    ...
    public void initialize() throws IOException {
        ...
        session.saveStageProperties(configuration, backstageResolved);
    }
    ...
    public FileNode localRepository() {
        return session.configuration.shared ? backstageResolved.join(".m2") : session.world.getHome().join(".m2/repository");
    }
